...
import { fromLamports, getTokenName, wadToLamports } from '@packages/common/utils/utils';
  ...
  useEffect(() => {
    const refreshTotal = () => {
      ...
      reserveAccounts.forEach((item) => {
        ...
        const liquidityMint = localCache.get(item.info.liquidityMint.toBase58()) as ParsedAccount<MintInfo>;
        ...
        let leaf = {
          ...
          borrowed:
            fromLamports(wadToLamports(item.info?.state.borrowedLiquidityWad).toNumber(), liquidityMint.info) * price,
