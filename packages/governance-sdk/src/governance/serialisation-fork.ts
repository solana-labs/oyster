...
const serializeInstructionToBase64 = (instruction: TransactionInstruction) => {
...
const createInstructionData = (instruction: TransactionInstruction) => {
...
function getGovernanceSchemaForAccount(accountType: GovernanceAccountType) {
  return getGovernanceAccountSchema(getGovernanceAccountVersion(accountType));
}

const GovernanceAccountParser = (classType: GovernanceAccountClass) =>
  BorshAccountParser(classType, (accountType: GovernanceAccountType) =>
    getGovernanceSchemaForAccount(accountType),
  );

function getInstructionDataFromBase64(instructionDataBase64: string) {
