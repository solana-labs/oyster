...
test('postProposalComment', async () => {
  ...
  const realmPk = await withCreateRealm(instructions, governanceProgramId, programVersion, name, realmAuthorityPk, mintPk, walletPk, undefined, MintMaxVoteWeightSource.FULL_SUPPLY_FRACTION, new BN(1), undefined)

  // Deposit governance tokens
  const tokenOwnerRecordPk = await withDepositGoverningTokens(instructions, governanceProgramId, programVersion, realmPk, ataPk, mintPk, walletPk, walletPk, walletPk, new BN(1));
  ...
  const governancePk = await withCreateMintGovernance(instructions, governanceProgramId, realmPk, mintPk, config, true, walletPk, tokenOwnerRecordPk, walletPk, walletPk, undefined)
  ...
  const proposalPk = await withCreateProposal(instructions, governanceProgramId, programVersion, realmPk, governancePk, tokenOwnerRecordPk, "proposal 1", "", mintPk, walletPk, 0, voteType, options, useDenyOption, walletPk)


  // Act

  const chatMessage = new ChatMessageBody({ type: ChatMessageBodyType.Text, value: "Let's do it" });

  const messagePk = await withPostChatMessage(instructions, signers, chatProgramId, governanceProgramId, realmPk, governancePk, proposalPk, tokenOwnerRecordPk, walletPk, walletPk, undefined, chatMessage)
